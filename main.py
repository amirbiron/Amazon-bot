...
# Event set when the bot thread dies — lets the main thread react.
_bot_fatal = threading.Event()
...
def _bot_loop():
    ...
    try:
        ...
        _bot_fatal.set()
        # Kill the whole process so Render detects the failure and restarts.
...
def main():
    ...
    if port:
        ...
        bot_thread = threading.Thread(target=_bot_loop, daemon=True, name="bot-loop")
        bot_thread.start()
